...
export default function LoginPage() {
  ...
  const [googleState, setGoogleState] = useState<"loading" | "enabled" | "disabled">("loading");
      ...
      <main className="mx-auto flex w-full max-w-lg flex-col gap-6 px-6 py-12">
        ...
        <div className="rounded-2xl border border-zinc-200 dark:border-white/10 bg-white dark:bg-white/5 p-6">
          ...
          <button
            ...
            className="flex w-full items-center justify-center gap-3 rounded-xl border border-zinc-300 dark:border-white/10 bg-white dark:bg-white/5 px-4 py-2.5 text-sm font-semibold text-zinc-900 dark:text-white transition-colors hover:bg-zinc-50 dark:hover:bg-white/10 disabled:cursor-not-allowed disabled:opacity-60"
          >
          ...
          {googleState === "disabled" && (
            <p className="mt-2 text-xs text-zinc-500 dark:text-zinc-400">
              Google sign-in isn&apos;t configured yet. Set GOOGLE_CLIENT_ID and GOOGLE_CLIENT_SECRET in .env and restart the dev server.
            </p>
          )}
        </div>
        ...
        <div className="rounded-2xl border border-zinc-200 dark:border-white/10 bg-white dark:bg-white/5 p-6">
          ...
          <Link href="/register" className="mt-4 inline-flex w-full items-center justify-center rounded-xl bg-emerald-600 px-4 py-2.5 text-sm font-semibold text-white transition-colors hover:bg-emerald-700">
            Create account
          </Link>
        </div>
      </main>
